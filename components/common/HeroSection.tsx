import React from 'react';
import { Button } from "@nextui-org/react";
import Link from 'next/link';

export default function HeroSection() {
  return (
    <section className="text-center py-20 bg-secondary/10 rounded-lg backdrop-blur-md border border-divider">
      <h1 className="text-5xl font-black text-primary mb-4">
        Bem‑vindo ao Squadra
      </h1>
      <p className="text-lg text-foreground/80 max-w-2xl mx-auto mb-8 font-medium">
        Crie times equilibrados de forma rápida e visualmente elegante. Explore as equipes geradas ou adicione seus próprios jogadores.
      </p>
      <Button
        as={Link}
        href="/teams"
        color="primary"
        size="lg"
        className="font-bold"
      >
        Ver Todas as Equipes
      </Button>
    </section>
  );
}
